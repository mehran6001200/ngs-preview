...
public final class VoiceEngine {
    ...
    public synchronized void stop(){
        running=false;
        if(thread!=null){ try{ thread.join(1000); }catch(InterruptedException e){ Thread.currentThread().interrupt(); } thread=null; }
    }

    private void loop(ErrorSink errors){
        Process.setThreadPriority(Process.THREAD_PRIORITY_URGENT_AUDIO);
        AudioRecord rec=null; AudioTrack out=null;
        try{
            ...
            short[] in=new short[1024]; short[] processed=new short[1024];
            rec.startRecording(); out.play();
            double phase=0.0;
            while(running){
                int n=rec.read(in,0,in.length,AudioRecord.READ_BLOCKING); if(n<=1) continue;
                double ratio=Math.pow(2.0,semitones/12.0);
                for(int i=0;i<n;i++){
                    double pos=phase+i*ratio;
                    int i0=((int)pos)%n; int i1=(i0+1)%n; double f=pos-Math.floor(pos);
        ...
        }catch(Throwable t){ if(errors!=null) errors.onError(t.getClass().getSimpleName()+": "+t.getMessage()); }
        finally{
            running=false;
            try{ if(rec!=null) rec.stop(); }catch(Throwable ignored){} try{ if(rec!=null) rec.release(); }catch(Throwable ignored){}
            try{ if(out!=null) out.stop(); }catch(Throwable ignored){} try{ if(out!=null) out.release(); }catch(Throwable ignored){}
        }
    }
}
